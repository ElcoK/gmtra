...
def calc_risk_total(RPS,loss_list):
    collect_risks = []
    for y in range(7):
...
def flood_wind_losses(x,design_table,depth_threshs,param_values,events,all_rps):
    ...
    for param in param_values:
        ...
        if x.road_type == 'primary':
            ...
            frag = np.array([0]*len(rps_not)+list((x[rps] > depth_thresh[0])*1))
            ...
            frag = np.array([0]*len(rps_not)+list((x[rps] > depth_thresh[1])*1))
            uncer_output.append((cost*x.length*param[0]*param[1]*4+cost*x.length*param[0]*(1-param[1])*2)*frag)
        else:
            ...
            frag = np.array([0]*len(rps_not)+list((x[rps] > depth_thresh[2])*1))
            uncer_output.append((cost*x.length*param[0]*param[1]*4+cost*x.length*param[0]*(1-param[1])*2)*frag)           
    
    return calc_risk_total(all_rps,list(zip(np.percentile(np.asarray(uncer_output), 0,axis=0),np.percentile(np.asarray(uncer_output), 20,axis=0),
np.percentile(np.asarray(uncer_output), 40,axis=0),np.percentile(np.asarray(uncer_output), 50,axis=0),np.percentile(np.asarray(uncer_output), 60,axis=0),
np.percentile(np.asarray(uncer_output), 80,axis=0),np.percentile(np.asarray(uncer_output), 100,axis=0))))

def earthquake_losses(x,eq_curve,param_values,vals_EQ,events,all_rps):
    
    uncer_output = []
    for param in param_values:
        curve = eq_curve.iloc[:,int(param[4])-1]
        frag = np.interp(list(x[[x for x in vals_EQ ]]),list(curve.index), curve.values)
        cost = x.cost[0]+((x.cost[1]-x.cost[0])*param[3])
...
def get_bridge_risk(file,data_path,param_values,income_lookup,eq_curve,design_tables,depth_threshs,wind_threshs):
    ...
    df['EQ_risk'] = df.progress_apply(lambda x: earthquake_losses(x,eq_curve,param_values,vals_EQ,events,all_rps),axis=1)
    ...
    df['FU_risk'] = df.progress_apply(lambda x: flood_wind_losses(x,design_table,depth_threshs,param_values,events,all_rps),axis=1)


    events = ['PU-5', 'PU-10', 'PU-20', 'PU-50', 'PU-75', 'PU-100', 'PU-200', 'PU-250','PU-500','PU-1000']
    tqdm.pandas(desc='Pluvial '+region)
    df['PU_risk'] = df.progress_apply(lambda x: flood_wind_losses(x,design_table,depth_threshs,param_values,events,all_rps),axis=1)
    ...
    df['Cyc_risk'] = df.progress_apply(lambda x: flood_wind_losses(x,design_table,wind_threshs,param_values,events,all_rps),axis=1)
...
def get_all_bridge_risks():
    ...
    eq_curve = pd.read_excel(os.path.join(data_path,'input_data','Costs_curves.xlsx'),sheet_name='bridge_curves',usecols=5)
    
    design_tables = np.array([[[1/200,1/200,1/200],[1/100,1/100,1/100],[1/50,1/50,1/50]],
                    [[1/100,1/100,1/100],[1/50,1/50,1/50],[1/20,1/20,1/20]],
                    [[1/50,1/50,1/50],[1/20,1/20,1/20],[1/10,1/10,1/10]]])
    
    depth_threshs = np.array([[700,600,500],[600,500,400],[500,400,300],[400,300,200]])
    wind_threshs = np.array([[400,375,350],[375,350,325],[350,325,300],[350,300,275]])    
    ...
    with Pool(40) as pool: 
        collect_risks = pool.starmap(get_bridge_risk,zip(all_files,data_p_list,param_list,income_list,eq_curve_list,design_list,depth_list,wind_list),chunksize=1) 
    
    
    pd.concat(collect_risks).to_csv(os.path.join(data_path,'summarized','bridge_risk_road.csv'))

def get_cost(x,events,param_values):
    uncer_output = []
    for param in param_values:
        ...
        uncer_output.append(np.asarray(uncer_events))

    x[events] = list(zip(np.percentile(np.asarray(uncer_output), 0,axis=0),np.percentile(np.asarray(uncer_output), 20,axis=0),
    np.percentile(np.asarray(uncer_output), 40,axis=0),np.percentile(np.asarray(uncer_output), 50,axis=0),np.percentile(np.asarray(uncer_output), 60,axis=0),
    np.percentile(np.asarray(uncer_output), 80,axis=0),np.percentile(np.asarray(uncer_output), 100,axis=0)))

    return x

def cyclone_damage(cycfil,data_path,events,param_values):
#    try:
    all_tree_cov = [os.path.join(data_path,'tree_cover_road',x) for x in os.listdir(os.path.join(data_path,'tree_cover_road'))]
    
    df_cyc = pd.read_feather(cycfil)
    ...
    if len(df_cyc) > 0:
        ...
        df_tree = pd.read_feather([x for x in all_tree_cov if os.path.split(cycfil)[1][:-7] in x][0])
        ...
        df_output = pd.DataFrame(columns=events,index=df_cyc.index).fillna(0)
        df_cyc = pd.concat([df_cyc,df_output],axis=1)
        tqdm.pandas(desc=region)

        df_cyc = df_cyc.progress_apply(lambda x: get_cost(x,events,param_values),axis=1)
...
def losses_parallel():  
    ...
    with Pool(6) as pool: 
        collect_output = pool.starmap(cyclone_damage,zip(all_cyc_road,data_paths,event_list,param_list),chunksize=1) 

    pd.concat(collect_output).to_csv(os.path.join(data_path,'summarized','Cyc_road_losses_uncer.csv'))


def loss_eq(x,global_costs,paved_ratios,frag_tables,events,wbreg_lookup,param_values,val_cols):
    ...
    lengths = np.array(x[[x for x in list(x.index) if 'length_' in x]])
...
def estimate_losses(file,data_path,global_costs,paved_ratios,events,wbreg_lookup):
    try:
        df = pd.read_feather(file)
        ...
        if len(df) > 0:
            df = df.rename({'val_EQ_rp250':'val_EQ_rp475',
                                        ...
                                        'length_EQ_rp1500':'length_EQ_rp2475',
                                        'length_EQ_rp2475':'length_EQ_rp975'}, axis='columns')
            df_liq = pd.read_feather(os.path.join(data_path,'liquefaction_road','{}_liq.ft'.format(region)))
        ...
        param_values = [np.fromfile(os.path.join(data_path,'input_data','param_values_eq.pkl'))[x:x+4] for x in range(0, len(np.fromfile(os.path.join(data_path,'input_data','param_values.pkl'))), 4)]
    
        sorted_all = [[10,25,40],[20,40,60],[30,55,80],[40,70,100]]
        frag_tables = []
        for sort_rand in sorted_all:
            EQ_fragility = np.array([[0]+sort_rand+[100],[0,0]+sort_rand,[0,0,0]+sort_rand[:2],[0,0,0,0]+sort_rand[:1],[0,0,0,0,0]]).T
            df_EQ_frag = pd.DataFrame(EQ_fragility,columns=[5,4,3,2,1],index=[(0,92),(93,180),(181,340),(341,650),(651,5000)])
        ...
        tqdm.pandas(desc = region)
        df = df.progress_apply(lambda x: loss_eq(x,global_costs,paved_ratios,frag_tables,events,wbreg_lookup,param_values,val_cols),axis=1)
...
def losses_parallel(hazard):  
    ...
    # import curves
    global_costs = pd.read_excel(os.path.join(data_path,'input_data','Costs_curves.xlsx'),usecols=7,header=0,index_col=0,skipfooter =45)
    global_costs.columns = ['SAS','SSA','MNA','EAP','LAC','ECA','YHI']
    
    paved_ratios = pd.read_csv(os.path.join(data_path,'input_data','paved_ratios.csv'),index_col=[0,1])
    ...
    shuffle(all_files)    
    with Pool(7) as pool: 
        pool.starmap(estimate_losses,zip(all_files,data_paths,pav_cost_list,pav_rat_list,events_list,wbreg_list),chunksize=1) 
...
def loss_estimations_flooding(x,global_costs,paved_ratios,flood_curve_paved,flood_curve_unpaved,events,wbreg_lookup,param_values,val_cols):
    ...
    uncer_output = []
    for param in param_values:
        paved_frag = np.interp(list(x[[x for x in val_cols if 'val' in x]]),list(curve_paved.index),(curve_paved.values)*param[2])
        unpaved_frag = np.interp(list(x[[x for x in val_cols if 'val' in x]]),list(curve_unpaved.index), curve_unpaved.values)
...
def estimate_losses(file,data_path,global_costs,paved_ratios,flood_curve_paved,flood_curve_unpaved,events,wbreg_lookup):
    try:
        df = pd.read_feather(file)
...
def losses_parallel(hazard,start_=0,end_=46233):  
    ...
    # import curves
    flood_curve_paved = pd.read_excel(os.path.join(data_path,'input_data','Costs_curves.xlsx'),usecols=[1,2,3,4,5,6,7,8],
                                 sheet_name='Flooding',index_col=[0],skipfooter=9,header = [0,1])
    flood_curve_paved.columns = flood_curve_paved.columns.droplevel(0)
    flood_curve_unpaved = pd.read_excel(os.path.join(data_path,'input_data','Costs_curves.xlsx'),usecols=[11,12,13,14,15,16,17,18],
                                 sheet_name='Flooding',index_col=[0],skipfooter=9,header = [0,1])
    ...
    with Pool(7) as pool: 
        collect_output = pool.starmap(estimate_losses,zip(all_files,data_paths,pav_cost_list,pav_rat_list,
                                                          cur_pav_list,cur_unpav_list,events_list,wbreg_list),chunksize=1) 

    pd.concat(collect_output).to_csv(os.path.join(data_path,'summarized','{}_road_losses_uncer_{}_{}.csv'.format(hazard,start_,end_)))
    
if __name__ == '__main__':
    losses_parallel(sys.argv[1],int(sys.argv[2]),int(sys.argv[3]))
