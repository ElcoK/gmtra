...
def create_hzd_df_fl(region,geometry,country_ISO3,hzd='FU'):
    ...
    flood_mapping = dict(zip(flood_type,[hzd]))
    flood_combis = list(product(flood_type, rps))

    geoms = [mapping(geometry)]

    all_hzds = []
    for flood_type,rp_ in tqdm(flood_combis,total=len(flood_combis),desc='Read Flood Maps {}'.format(region)):
        flood_type_abb = flood_mapping[flood_type]
        flood_scen = '{}-{}'.format(flood_type_abb,rp_)

        flood_path = os.path.join(hazard_path,'InlandFlooding',country_full,'{}_{}_merged'.format(country_ISO2,flood_type),'{}-{}-{}.tif'.format(country_ISO2,flood_type_abb,rp_))    
...
def intersect_hazard(x,EQ_reg_sindex,EQ_region):
    matches = EQ_region.iloc[list(EQ_reg_sindex.intersection(x.geometry.bounds))].reset_index(drop=True)
    try:
        if len(matches) == 0:
            return x.geometry,0
        else:
...
def fetch_hazard_values(n,hzd,road=True,railway=False):
    ...
    try:
        if (road == True) & os.path.exists(os.path.join(data_path,'output_{}_full'.format(hzd),'{}_{}.ft'.format(region,hzd))):
        ...
        elif (railway == True) & os.path.exists(os.path.join(data_path,'output_{}_rail_full'.format(hzd),
        ...
        try:
            if road == True:
                road_gpd = fetch_roads(data_path,region,regional=True)
                road_dict = map_roads()
                ...
                rail_gpd = fetch_railway(data_path,region,regional=True)
...
def intersect_liquefaction(x,EQ_reg_sindex,EQ_region):
    matches = EQ_region.iloc[list(EQ_reg_sindex.intersection(x.geometry.bounds))].reset_index(drop=True)
    if len(matches) == 0:
        return x.geometry,0
    else:
...
def get_liquefaction_region(x,road=True):
    region = x[3]
    reg_geom = x[-1]
    data_path = load_config()['paths']['data']
    try:
        if (road == True) & os.path.exists(os.path.join(data_path,'liquefaction_road','{}_liq.ft'.format(region))):
            print('{} already finished!'.format(region))
            return None
        ...
        inb = infra_gpd.progress_apply(lambda x: intersect_hazard(x,gdf.sindex,gdf),axis=1).copy()
        inb = inb.apply(pd.Series)
...
def get_tree_density(x,road =False):
...
def region_bridges(x,road=True):
    ...
    try:
        ...
        bridges_osm = fetch_bridges(data_path,region,regional=True)
